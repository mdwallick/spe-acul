  ...
  return (
    <div className="space-y-2 min-h-[120px]">
      ...
      <ul className="text-sm text-gray-600 space-y-1">
        <li
          className={`flex items-center ${hasMinLength(watchedPassword) ? "text-green-600" : "text-gray-500"}`}
        >
          <span className="mr-2">
        ...
        <li
          className={`flex items-center ${hasNumber(watchedPassword) ? "text-green-600" : "text-gray-500"}`}
        >
          <span className="mr-2">{hasNumber(watchedPassword) ? "✓" : "○"}</span>
          At least one number
        </li>
        <li
          className={`flex items-center ${hasLetter(watchedPassword) ? "text-green-600" : "text-gray-500"}`}
        >
          <span className="mr-2">{hasLetter(watchedPassword) ? "✓" : "○"}</span>
          At least one letter
        </li>
        <li
          className={`flex items-center ${hasSpecialChar(watchedPassword) ? "text-green-600" : "text-gray-500"}`}
        >
          <span className="mr-2">
        ...
        <li
          className={`flex items-center ${!hasEmailPortion(watchedPassword, watchedEmail) ? "text-green-600" : "text-gray-500"}`}
        >
          <span className="mr-2">
            {!hasEmailPortion(watchedPassword, watchedEmail) ? "✓" : "○"}
          </span>
